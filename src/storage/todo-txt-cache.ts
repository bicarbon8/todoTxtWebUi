...
export class TodoTextPersistance {
    tasks: TaskItem[] = [];
    config: TodoTxtConfig = { showClosed: false };

    constructor(tasks?: Map<string, TodoTxtTask>, config?: TodoTxtConfig) {
        if (tasks) {
            tasks.forEach((value: TodoTxtTask) => {
                this.tasks.push({id: value.id, text: value.text});
            });
        }
        if (config) {
            this.config = config;
